...
import { useSplitPart, useSplitPartItem, useExecSplitPartTransactions, useUpdateSplitPart } from '../../hooks/useSplitPart';

import { styles } from '../styles';

const SplitPart: React.FC = () => {
  ...
  const { isLoading: orderIsLoading, isFetching, isError, data: orderData, refetch: orderRefetch, status, error } = useSplitPart();
  ...
  const validateErrors = () => {
    ...
    if (!order.Rec_ID) {
      setErrors({ ...errors, Rec_ID: 'Split Order is required' });
      clearState('Item');
      return false;
    }
            ...
            <VStack space={10} p={5}>
              <FormControl isRequired isInvalid={'Rec_ID' in errors}>
                <Select
                  ...
                  placeholder="SPLIT PART ORDER NO."
